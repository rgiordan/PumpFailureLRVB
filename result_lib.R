...
GetMCMCLambdaGroupResults <- function(ind, mcmc_draws) {
  return(rbind(
    GetParamRow(value=mean(mcmc_draws$lambda[, ind]), par="lambda", group=ind, method="mcmc", metric="mean"),
    GetParamRow(value=sd(mcmc_draws$lambda[, ind]), par="lambda", group=ind, method="mcmc", metric="sd")
  ))
}
  ...
  for (ind in 1:ncol(lambda_draws)) {
    for (prior_ind in 1:length(prior_param_names)) {
      result_list[[length(result_list) + 1]] <-
        GetParamRow(value=sens[ind, prior_ind], par="lambda", group=ind, method="mcmc",
...
GetMCMCBetaSensitivityResults <- function(mcmc_draws, log_prior_grad_mat, prior_param_names) {
  ...
  for (prior_ind in 1:length(prior_param_names)) {
    result_list[[length(result_list) + 1]] <-
      GetParamRow(value=sens[1, prior_ind], par="beta", method="mcmc", metric=prior_param_names[prior_ind])
  }
  return(do.call(rbind, result_list))
}
...
GetVBLambdaShape <- function(ind, vb_fit) { vb_fit$par[[paste("lambda_shape[", ind, "]", sep="")]] }
GetVBLambdaRate <- function(ind, vb_fit) { vb_fit$par[[paste("lambda_rate[", ind, "]", sep="")]] }
...
GetVBLambdaGroupResults <- function(ind, vb_fit) {
  ...
  return(rbind(
    GetParamRow(value=lambda_shape / lambda_rate, par="lambda", group=ind, method="mfvb", metric="mean"),
    GetParamRow(value=sqrt(lambda_shape) / lambda_rate, par="lambda", group=ind, method="mfvb", metric="sd")
  ))
}
...
GetVBLambdaStarShape <- function(ind, vb_fit) { vb_fit$par[[paste("lambda_star_shape[", ind, "]", sep="")]] }
GetVBLambdaStarRate <- function(ind, vb_fit) { vb_fit$par[[paste("lambda_star_rate[", ind, "]", sep="")]] }
